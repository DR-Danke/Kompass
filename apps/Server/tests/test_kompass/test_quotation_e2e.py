"""End-to-end tests for Quotation workflows.

Tests cover:
- Full CRUD workflow (Create -> Read -> Update -> Delete)
- Line item management (add, remove, update)
- Quotation cloning
- Pricing calculation (full formula validation)
...
class TestQuotationCRUDWorkflow:
    """Tests for complete CRUD workflow."""

    def test_create_read_flow(
        self, quotation_service, sample_quotation_data
    ):
        """Test quotation creation and retrieval."""
        quotation_service.repository.create.return_value = sample_quotation_data
        quotation_service.repository.get_by_id.return_value = sample_quotation_data

        request = QuotationCreateDTO(
            client_id=sample_quotation_data["client_id"],
        ...
        result = quotation_service.create_quotation(
            request=request,
            created_by=sample_quotation_data["created_by"],
        )
    ...
    def test_create_quotation_with_items(
        self, quotation_service, sample_quotation_with_items
    ):
        """Test creating a quotation with initial items."""
        quotation_service.repository.create.return_value = sample_quotation_with_items
        quotation_service.repository.get_by_id.return_value = sample_quotation_with_items
        quotation_service.repository.add_item.return_value = sample_quotation_with_items["items"][0]

        request = QuotationCreateDTO(
            client_id=sample_quotation_with_items["client_id"],
            items=[
                QuotationItemCreateDTO(
        ...
        result = quotation_service.create_quotation(
            request=request,
            created_by=sample_quotation_with_items["created_by"],
        )
    ...
    def test_get_quotation_found(
        self, quotation_service, sample_quotation_data
    ):
        """Test getting a quotation by ID."""
        quotation_service.repository.get_by_id.return_value = sample_quotation_data

        result = quotation_service.get_quotation(sample_quotation_data["id"])

        assert result is not None
        assert result.id == sample_quotation_data["id"]
...
class TestQuotationItems:
    """Tests for line item management."""

    def test_add_item_success(self, quotation_service, sample_quotation_data):
        """Test adding line item triggers add operation."""
        mock_item = {
            "id": uuid4(),
            "quotation_id": sample_quotation_data["id"],
            "product_id": None,
            "sku": "SKU001",
            "product_name": "Widget A",
        ...
        result = quotation_service.add_item(sample_quotation_data["id"], item_request)
    ...
    def test_remove_item_success(self, quotation_service):
        """Test removing item returns True."""
        ...
        quotation_service.repository.remove_item.assert_called_once_with(item_id)
...
class TestQuotationCloning:
    ...
    def test_clone_quotation_success(
        ...
        result = quotation_service.clone_quotation(
            quotation_id=sample_quotation_with_items["id"],
            created_by=uuid4(),
        )
    ...
    def test_clone_quotation_with_notes(
        self, quotation_service, sample_quotation_data
    ):
        """Test cloning a quotation with custom notes."""
        cloned = sample_quotation_data.copy()
        cloned["id"] = uuid4()
        cloned["quotation_number"] = "QT-000002"

        quotation_service.repository.get_by_id.side_effect = [
            sample_quotation_data, cloned
        ]
        quotation_service.repository.create.return_value = cloned
...
class TestPricingCalculation:
    ...
    def test_calculate_pricing_formula(
        self, mock_pricing_service, quotation_service, sample_quotation_with_items
    ):
        ...
        result = quotation_service.calculate_pricing(sample_quotation_with_items["id"])
    ...
    def test_calculate_pricing_empty_quotation(
        self, mock_pricing_service, quotation_service, sample_quotation_data
    ):
        ...
        result = quotation_service.calculate_pricing(sample_quotation_data["id"])
...
class TestStatusWorkflow:
    ...
    def test_update_status_success(
        ...
        quotation_service.repository.get_by_id.return_value = sample_quotation_data
        updated = sample_quotation_data.copy()
        updated["status"] = "sent"
        ...
        result = quotation_service.update_status(sample_quotation_data["id"], transition)
    ...
    def test_update_status_invalid_transition(
        ...
        with pytest.raises(ValueError) as exc_info:
            quotation_service.update_status(sample_quotation_data["id"], transition)
...
class TestPDFGeneration:
    ...
    def test_generate_pdf_success(
        ...
        result = quotation_service.generate_pdf(sample_quotation_with_items["id"])
    ...
    def test_generate_pdf_empty_quotation(
        self, quotation_service, sample_quotation_data
    ):
        """Test generating PDF for quotation with no items."""
        quotation_service.repository.get_by_id.return_value = sample_quotation_data

        result = quotation_service.generate_pdf(sample_quotation_data["id"])
...
class TestEmailSending:
    ...
    def test_send_email_mock_mode(
        ...
        result = quotation_service.send_email(sample_quotation_data["id"], request)
    ...
    @patch.dict("os.environ", {"EMAIL_MOCK_MODE": "true"})
    def test_send_email_without_pdf(
        self, quotation_service, sample_quotation_data
    ):
...
class TestShareToken:
    ...
    def test_get_share_token_success(
        self, mock_settings, sample_quotation_data
    ):
        ...
        service = QuotationService()
        service.repository = MagicMock()
        service.repository.get_by_id.return_value = sample_quotation_data

        result = service.get_share_token(sample_quotation_data["id"])

        assert result is not None
        assert result.token is not None
        assert len(result.token) > 0
        assert result.quotation_id == sample_quotation_data["id"]
    ...
    @patch("app.services.quotation_service.get_settings")
    def test_get_by_share_token_success(self, mock_settings, sample_quotation_data):
        ...
        token_result = service.get_share_token(sample_quotation_data["id"])
        ...
        assert result is not None
        assert result.id == sample_quotation_data["id"]
        assert result.quotation_number == sample_quotation_data["quotation_number"]
...
class TestListQuotations:
    ...
    def test_list_quotations_with_filters(
        self, quotation_service, sample_quotation_data
    ):
        """Test listing quotations with filters."""
        quotation_service.repository.get_all.return_value = ([sample_quotation_data], 1)
...
class TestUpdateQuotation:
    ...
    def test_update_quotation_success(
        self, mock_close, mock_get_conn, quotation_service, sample_quotation_data
    ):
        ...
        mock_cursor.fetchone.return_value = (sample_quotation_data["id"],)
        mock_get_conn.return_value = mock_conn

        quotation_service.repository.get_by_id.return_value = sample_quotation_data
        updated_quotation = sample_quotation_data.copy()
        ...
        result = quotation_service.update_quotation(sample_quotation_data["id"], request)
...
class TestDeleteQuotation:
    ...
    def test_delete_quotation_success(
        ...
        result = quotation_service.delete_quotation(sample_quotation_data["id"])
