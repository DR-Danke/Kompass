...
DEFAULT_EXAMPLE_PATH = os.path.join(
    ...
    "ai_docs",
    "SERVICEDESK_ADW_IMPLEMENTATION_PROMPTS_WITH_CONTEXT.md",
)
...
def main():
    ...
    src_cmd = os.path.join(main_repo_root, ".claude", "commands", "prd_to_prompts.md")
    dst_cmd = os.path.join(worktree_path, ".claude", "commands", "prd_to_prompts.md")
    if os.path.exists(src_cmd) and not os.path.exists(dst_cmd):
        ...
        logger.info("Synced prd_to_prompts.md slash command into worktree")
    ...
    if "unknown skill" in response.output.lower():
        ...
        print("ERROR: /prd_to_prompts slash command not found in worktree.")
        sys.exit(1)
